...
const routes = [
	{
		path: "/",
		name: "PayDashboard",
		component: () => import("./PayDashboard.vue"),
	},
	{
		name: "Login",
		path: "/login",
...
const router = createRouter({
	history: createWebHistory("/pay-dashboard"),
	routes,
})
...
setNavigationCallbacks({
	onLoginSuccess: (defaultRoute) => {
		...
		if (defaultRoute && !defaultRoute.includes('/jsapp/') && defaultRoute.startsWith('/pay-dashboard')) {
			// Extract relative path from /pay-dashboard/...
			targetRoute = defaultRoute.replace('/pay-dashboard', '') || "/"
...
router.beforeEach(async (to, from, next) => {
	...
	if (to.name === "Login" && isLoggedIn) {
		next(intendedRoute || { name: "PayDashboard" })
		intendedRoute = null
