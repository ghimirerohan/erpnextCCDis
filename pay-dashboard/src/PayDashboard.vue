    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8">
      ...
      <section class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sm:p-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-6">
          <div class="space-y-2">
            ...
            <div class="text-xs sm:text-sm uppercase text-gray-600 tracking-wide mb-2">Select Date</div>
            <NepaliDatePicker
            ...
            <div class="pt-2 border-t border-gray-200 mt-3">
              <div class="text-xs sm:text-sm uppercase text-gray-600 tracking-wide mb-1">{{ selectedDate ? 'Selected Date' : "Today's" }} Total Success</div>
      ...
      <section class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sm:p-8">
        ...
        <div v-else-if="!hasData" class="text-center py-12">
          ...
          <p class="mt-4 text-gray-600">No data found{{ selectedDate ? ` for ${selectedDateBs}` : ' for today' }}.</p>
        </div>
              ...
              >
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                  ...
                  <div>
                    ...
                    <div class="text-sm font-medium text-gray-900">{{ formatDateTime(txn.creation || txn.posting_date) }}</div>
                    <div class="text-xs text-blue-600 mt-1">
                      <a 
                        :href="`/app/payment-entry/${txn.payment_entry || txn.name}`" 
                        target="_blank" 
...
// Handle date change - reload all data when date is selected
const handleDateChange = async (adDate) => {
  selectedDate.value = adDate
  // Reload all data with the new date filter
  await refreshAll()
}

// Clear date filter and reset to today
const clearDateFilter = async () => {
  selectedDate.value = null
  // Update today's BS date
  await tryLoadNepaliScriptAndSetBSToday()
  await refreshAll()
}
  ...
  try {
    ...
    for (const pe of paymentEntries) {
      ...
      if (!peByOwner[owner]) {
        peByOwner[owner] = {
          username: owner,
          full_name: pe.owner_full_name || owner,
          total_amount: 0,
          count: 0,
        }
      }
      peByOwner[owner].total_amount += pe.amount
...
const loadCustomerData = async () => {
  ...
  try {
    ...
    const peByCustomer = {}
    for (const pe of paymentEntries) {
      if (!pe.customer) continue
      if (selectedCustomer.value?.value && pe.customer !== selectedCustomer.value.value) {
        continue
      }
      if (selectedUsername.value && pe.owner !== selectedUsername.value) {
        continue
      ...
      if (!peByCustomer[customer]) {
        peByCustomer[customer] = {
          customer: customer,
          customer_name: pe.customer_name || customer,
          total_amount: 0,
          count: 0,
        }
      }
      peByCustomer[customer].total_amount += pe.amount
...
const refreshAll = async () => {
  loading.value = true
  try {
    // Update today's date if no date filter is selected
