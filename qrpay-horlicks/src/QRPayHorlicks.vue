      ...
      <section class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 sm:p-8">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 lg:gap-8">
          ...
          <div class="flex flex-col sm:flex-row gap-3 flex-shrink-0">
            ...
            <button
              @click="$router.push('/previous-transactions')"
              class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-lg bg-white hover:bg-gray-50"
            >
              <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          <div v-if="selectedCustomer" class="space-y-3">
            <div class="flex items-center justify-between">
              ...
              <button
                @click="clearCustomer"
                class="inline-flex items-center px-3 py-1.5 text-sm font-bold rounded-lg shadow-md transition-all border-2"
                style="background-color: #dc2626; color: white; border-color: #b91c1c;"
...
const clearCustomer = () => {
  ...
  // Also clear the CustomerSearch component's internal state
  if (customerSearchRef.value) {
    customerSearchRef.value.clear()
  }
  pushStatusLog('info', 'Customer selection cleared')
}
