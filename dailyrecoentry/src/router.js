	...
	if (!currentPath.includes('/login') && !currentPath.includes('/account/login')) {
		...
		if (router && router.currentRoute.value.name !== "Login") {
			...
			try {
				...
				window.location.replace('/dailyrecoentry/login')
			}
		} else {
			window.location.replace('/dailyrecoentry/login')
		}
...
const routes = [
	{
		path: "/",
		name: "DailyRecoEntry",
		component: () => import("./DailyRecoEntry.vue"),
	},
	{
		name: "CustomerPayment",
		path: "/customer/:lineName",
		component: () => import("./CustomerPayment.vue"),
	},
	{
		name: "Login",
		path: "/login",
...
router = createRouter({
	history: createWebHistory("/dailyrecoentry"),
	routes,
})
...
setNavigationCallbacks({
	onLoginSuccess: (defaultRoute) => {
		...
		if (defaultRoute && !defaultRoute.includes('/jsapp/') && defaultRoute.startsWith('/dailyrecoentry')) {
			// Extract relative path from /dailyrecoentry/...
			targetRoute = defaultRoute.replace('/dailyrecoentry', '') || "/"
...
router.beforeEach(async (to, from, next) => {
	...
	if (to.name === "Login" && isLoggedIn) {
		...
		next(intendedRoute || { name: "DailyRecoEntry" })
		intendedRoute = null
